...
# Author: Martin Blaho
# Description: Metop - AHRPT receive and viterbi decode signal to CADU -use baseband file as input
# Generated: Sat Jan 22 12:00:14 2011
##################################################
...
class file_rx_metop_ahrpt_fr(grc_wxgui.top_block_gui):
...
if __name__ == '__main__':
	...
	parser.add_option("", "--frames-file", dest="frames_file", type="string", default=os.environ['HOME'] + '/MetOp-AHRPT_cadu_frames.cadu',
	...
	tb = file_rx_metop_ahrpt_fr(deframer_insync_frames=options.deframer_insync_frames, viterbi_insync_frames=options.viterbi_insync_frames, deframer_outsync_frames=options.deframer_outsync_frames, viterbi_outsync_frames=options.viterbi_outsync_frames, viterbi_sync_threshold=options.viterbi_sync_threshold, pll_alpha=options.pll_alpha, freq=options.freq, gain=options.gain, side=options.side, decim=options.decim, symb_rate=options.symb_rate, frames_file=options.frames_file, clock_alpha=options.clock_alpha, baseband_file=options.baseband_file, satellite=options.satellite)
	tb.Run(True)
