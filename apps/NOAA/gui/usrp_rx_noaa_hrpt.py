...
# Generated: Thu Jan  6 14:32:42 2011
##################################################
...
class usrp_rx_noaa_hrpt(grc_wxgui.top_block_gui):

	def __init__(self, satellite='NOAA-XX', frames_file=os.environ['HOME'] + '/NOAA-XX.hrpt', side="A", gain=35, freq=1698e6, decim=32, sync_check=False):
...
if __name__ == '__main__':
	...
	tb = usrp_rx_noaa_hrpt(satellite=options.satellite, frames_file=options.frames_file, side=options.side, gain=options.gain, freq=options.freq, decim=options.decim, sync_check=options.sync_check)
	tb.Run(True)
