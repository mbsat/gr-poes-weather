...
# Author: Martin Blaho
# Description: GOES - LRIT receive to baseband and decode signal to CADU
# Generated: Wed Jan 12 23:57:27 2011
##################################################
...
class usrp_rx_goes_lrit_bb_fr(grc_wxgui.top_block_gui):

	def __init__(self, viterbi_sync_check=True, viterbi_insync_frames=5, deframer_insync_frames=2, deframer_outsync_frames=5, viterbi_outsync_frames=20, deframer_sync_check=True, pll_alpha=0.005, symb_rate=293883, clock_alpha=0.005, viterbi_sync_threshold=0.1, baseband_file=os.environ['HOME'] + '/GOES-LRIT_baseband.dat', frames_file=os.environ['HOME'] + '/GOES-LRIT_cadu_frames.cadu', satellite='GOES-LRIT', freq=1691.02e6, gain=23, decim=108, side="A"):
		...
		self.gr_file_sink_0 = gr.file_sink(gr.sizeof_gr_complex*1, baseband_file)
		...
		self.connect((self.usrp_simple_source, 0), (self.gr_file_sink_0, 0))
	...
	def set_decim_tb(self, decim_tb):
		...
		self.usrp_simple_source.set_decim_rate(self.decim_tb)
	...
	def set_gain_tb(self, gain_tb):
		...
		self.usrp_simple_source.set_gain(self.gain_tb)

	def set_freq_tb(self, freq_tb):
		self.freq_tb = freq_tb
		self._freq_tb_text_box.set_value(self.freq_tb)
		self.usrp_simple_source.set_frequency(self.freq_tb)
...
if __name__ == '__main__':
	...
	tb = usrp_rx_goes_lrit_bb_fr(viterbi_insync_frames=options.viterbi_insync_frames, deframer_insync_frames=options.deframer_insync_frames, deframer_outsync_frames=options.deframer_outsync_frames, viterbi_outsync_frames=options.viterbi_outsync_frames, pll_alpha=options.pll_alpha, symb_rate=options.symb_rate, clock_alpha=options.clock_alpha, viterbi_sync_threshold=options.viterbi_sync_threshold, baseband_file=options.baseband_file, frames_file=options.frames_file, satellite=options.satellite, freq=options.freq, gain=options.gain, decim=options.decim, side=options.side)
	tb.Run(True)
